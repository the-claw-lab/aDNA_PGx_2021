...
#Paths
preffixPath <- "~/Desktop/2021_Archaic_PGx_example/" #"~/path/to/working/dir/"
SGEPath = "~/Software/Stargazer/" #"~/path/to/Stargazer/"

#########Import Data########
geneList <- c("cyp1a2", "cyp2a6", "cyp2b6", "cyp2c8", "cyp2c9", "cyp2c19", "cyp2d6", "cyp2e1", "cyp2j2", "cyp3a4", "cyp3a5")
controlGenes <- c("vdr", "egfr", "ryr1") #can be vdr, egfr, or ryr1
...
for(gene in geneList){
  for(control in controlGenes){
    path <- paste(preffixPath, "sge_output/sge_output_full_", control, "/", gene, "/full_", gene, "_", control, ".stargazer-genotype.txt", sep = "")
...
#Read Depth Data
for(gene in geneList){
  path <- paste(preffixPath, "depth_of_coverage/neanderthal_vdr_", gene, "_out.gdf", sep = "")
...
#Manually change vinidija cyp2c9 call to *1/*1
  ...
  if(main.1 == main.2){
    ...
    if(core != "."){
      ...
      for(var in cor.vars){
        ...
        # #Get genotype call
        # geno <- filter(variant_info_table, variant_info_table$Gene == gene)
        # geno <- filter(geno, geno$POS == pos)
        # geno <- filter(geno, geno$Sample == sample)
        # genotype <- geno$Genotype[1]
        # 
        # #Get allelic depth
        # AD <- geno$Genotype_AllelicDepth[1]
        # 
        # #Get reference and alternative
        # REF_ALT <- paste(geno$REF[1], ",", geno$ALT[1], sep = "")
